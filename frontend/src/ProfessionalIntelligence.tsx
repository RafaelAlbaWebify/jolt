...
export function ProfessionalIntelligence({ apiBase, active }: Props) {
  ...
  const [capturePlanRevision, setCapturePlanRevision] = useState(0);
  ...
  async function runSourceAction(sourceId: string, path: string, body?: SourceUpdate) {
    ...
    try {
      ...
      setCapturePlanRevision((current) => current + 1);
    } catch (caught) {
  ...
  return (
    <main className="professional-intelligence" aria-labelledby="professional-intelligence-heading">
      <section className="panel professional-intelligence-overview">
        <div>
          <p className="eyebrow">Capture / intake</p>
          <h2 id="professional-intelligence-heading">Sources & Evidence</h2>
          <p>
            Start the user-present LinkedIn capture workflow, then review the resulting local evidence.
            Manual job entry remains only a fallback in Review Inbox.
          </p>
        </div>
        <div className="professional-overview-actions">
          <div className="professional-capture-controls" aria-label="Capture settings">
            <div className="professional-capture-settings-grid">
              <label>
                Maximum sources
                <input type="number" min="1" max="12" value={captureOptions.max_sources} onChange={(event) => setNumericCaptureOption("max_sources", event.target.value)} />
              </label>
              <label>
                Scroll batches per source
                <input type="number" min="0" max="20" value={captureOptions.max_scroll_batches} onChange={(event) => setNumericCaptureOption("max_scroll_batches", event.target.value)} />
              </label>
              <label>
                Maximum items per source
                <input type="number" min="1" max="200" value={captureOptions.max_items_per_source} onChange={(event) => setNumericCaptureOption("max_items_per_source", event.target.value)} />
              </label>
              <label>
                Timeout per source (seconds)
                <input type="number" min="10" max="120" value={captureOptions.timeout_seconds} onChange={(event) => setNumericCaptureOption("timeout_seconds", event.target.value)} />
              </label>
            </div>
            <label className="professional-source-checkbox">
              <input
                ...
                onChange={(event) => setCaptureOptions((current) => ({ ...current, stop_on_failure: event.target.checked }))}
              />
              Stop the run after the first failed source.
            </label>
            <button type="button" disabled={!active} aria-controls="professional-run-ledger" onClick={() => setCaptureStartRequestKey((current) => current + 1)}>
              Start capture
            </button>
            <span>Opens the visible user-present capture workflow and records a governed local evidence batch.</span>
          </div>
      ...
      {active && (
        <ProfessionalCaptureRuns
          apiBase={apiBase}
          active={active}
          planRefreshKey={capturePlanRevision}
