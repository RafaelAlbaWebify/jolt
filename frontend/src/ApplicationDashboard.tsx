...
type ApplicationEvent = { event_id: string; event_type: string; from_status: string; to_status: string; notes: string; occurred_at: string };
type ApplicationDetail = { application_id: string; posting_id: string; status: ApplicationStatus; application_url: string; resume_used: string; notes: string; outcome_type: string | null; events: ApplicationEvent[] };
...
const TABS: Array<{ id: WorkspaceTab; label: string }> = [
  { id: "overview", label: "Overview" }, { id: "tasks", label: "Tasks" }, { id: "interviews", label: "Interviews" },
  { id: "contacts", label: "Contacts" }, { id: "documents", label: "Documents" }, { id: "timeline", label: "Timeline" },
];
...
function displayDate(value: string | null | undefined) { return value ? new Date(value).toLocaleString() : "None scheduled"; }
function Placeholder({ title, copy }: { title: string; copy: string }) { return <section className="application-tab-placeholder"><h4>{title}</h4><p>{copy}</p></section>; }
function Timeline({ detail, loading }: { detail: ApplicationDetail | null; loading: boolean }) {
  ...
  return <section className="application-timeline" aria-labelledby="application-timeline-heading">
    <div className="application-tab-heading"><div><p className="eyebrow">Immutable activity history</p><h4 id="application-timeline-heading">Timeline</h4></div><span>{events.length} events</span></div>
...
export function ApplicationDashboard({ apiBase, active }: Props) {
  ...
  useEffect(() => { if (active) refresh().catch((caught) => setError(caught instanceof Error ? caught.message : "Application dashboard failed.")); }, [active, refresh]);
  useEffect(() => {
  ...
  const candidates = useMemo(() => opportunities.filter((item) => item.review_decision === "pursue" || Boolean(item.application_id)), [opportunities]);
  ...
  const grouped = useMemo(() => Object.fromEntries(LANES.map((lane) => [lane.id, visibleCandidates.filter((item) => laneFor(item) === lane.id)])) as Record<PipelineLane, Opportunity[]>, [visibleCandidates]);
  ...
  const loadApplicationDetail = useCallback(async (applicationId: string | null | undefined) => {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Application history failed."); }
    finally { setDetailLoading(false); }
  }, [apiBase]);
  useEffect(() => { setActiveTab("overview"); setApplicationDetail(null); if (selected) void loadApplicationDetail(selected.application_id); }, [loadApplicationDetail, selected?.application_id, selected?.posting_id]);
  async function refreshAfterChange() { setBusy(true); try { await refresh(); if (selected?.application_id) await loadApplicationDetail(selected.application_id); } finally { setBusy(false); } }
  function openWorkspace(postingId: string) { setActiveTab("overview"); setSelectedPostingId(postingId); }

  return <section className="panel application-workspace" aria-labelledby="application-dashboard-heading">
    <div className="section-heading application-workspace-heading"><div><p className="eyebrow">Application pipeline</p><h2 id="application-dashboard-heading">Application management</h2><p>Move from preparation to applied, interviews, offer, and closure without losing history.</p></div><button type="button" className="secondary" disabled={busy} onClick={() => refresh().catch(() => setError("Unable to refresh applications."))}>Refresh applications</button></div>
    <div className="application-board-toolbar"><label className="application-search"><span>Search pipeline</span><input type="search" value={query} placeholder="Role, company, location, or stage" onChange={(event) => setQuery(event.target.value)} /></label><p className="application-boundary">JOLT records the workflow. Applications and recruiter contact remain under your control.</p></div>
    {error && <p className="error" role="alert">{error}</p>}
    <div className="application-board" aria-label="Application pipeline board">{LANES.map((lane) => <section className={`application-lane application-lane-${lane.id}`} key={lane.id} aria-labelledby={`lane-${lane.id}`}>
      ...
      <div className="application-lane-cards">{grouped[lane.id].length === 0 ? <p className="application-lane-empty">No applications</p> : grouped[lane.id].map((opportunity) => <article className={`application-card${opportunity.overdue ? " application-card-overdue" : ""}`} key={opportunity.posting_id}>
        <button type="button" className="application-card-open" onClick={() => openWorkspace(opportunity.posting_id)} aria-label={`Open ${opportunity.title || "untitled opportunity"}`}>
          <span className="application-card-stage">{label(opportunity.outcome_type ?? opportunity.application_status)}</span>{opportunity.overdue && <span className="application-card-alert">Overdue</span>}<strong>{opportunity.title || "Untitled opportunity"}</strong><span className="application-card-company">{opportunity.company || "Unknown company"}</span>{opportunity.location && <span className="application-card-location">{opportunity.location}</span>}
          <dl className="application-card-signals"><div><dt>Last activity</dt><dd>{displayDate(opportunity.last_activity_at)}</dd></div><div><dt>Next {opportunity.next_due_kind ?? "due"}</dt><dd>{displayDate(opportunity.next_due_at)}</dd></div><div><dt>Documents</dt><dd>{opportunity.document_state ?? "Unknown"}</dd></div></dl>
          <span className="application-card-next"><b>Next:</b> {nextAction(opportunity)}</span>
        </button><div className="application-card-links">{opportunity.source_url && <a href={opportunity.source_url} target="_blank" rel="noreferrer">Source job</a>}<a href={`${apiBase}/api/opportunities/${opportunity.posting_id}/preparation-pack`} download>Preparation pack</a></div>
      </article>)}</div>
    </section>)}</div>
    {selected && <div className="application-workspace-overlay" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedPostingId(null); }}><section className="application-detail-workspace" role="dialog" aria-modal="true" aria-labelledby="application-detail-title">
      ...
      <nav className="application-detail-tabs" aria-label="Application workspace sections" role="tablist">{TABS.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`application-panel-${tab.id}`} className={activeTab === tab.id ? "application-detail-tab-active" : ""} onClick={() => setActiveTab(tab.id)}>{tab.label}</button>)}</nav>
