    ...
    <div className="application-board" aria-label="Application pipeline board">{LANES.map((lane) => <section className={`application-lane application-lane-${lane.id}`} key={lane.id} aria-labelledby={`lane-${lane.id}`}>
      ...
      <div className="application-lane-cards">{grouped[lane.id].length === 0 ? <p className="application-lane-empty">No applications</p> : grouped[lane.id].map((opportunity) => <article className={`application-card${opportunity.overdue ? " application-card-overdue" : ""}`} key={opportunity.posting_id}>
        <button type="button" className="application-card-open" onClick={() => openWorkspace(opportunity.posting_id)} aria-label={`Open ${opportunity.title || "untitled opportunity"}`}>
          ...
          <dl className="application-card-signals"><div><dt>Last activity</dt><dd>{displayDate(opportunity.last_activity_at)}</dd></div><div><dt>Next {opportunity.next_due_kind ?? "due"}</dt><dd>{displayDate(opportunity.next_due_at)}</dd></div><div><dt>Resume</dt><dd>{opportunity.document_state ?? "Unknown"}</dd></div></dl>
