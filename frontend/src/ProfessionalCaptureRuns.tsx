...
export function ProfessionalCaptureRuns({
  ...
  const loadRuns = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Unable to load the capture run ledger.");
        ...
        <div className="professional-run-list">
          {runs.map((run) => (
            <article className="professional-run-card" key={run.id}>
              <div><strong>{humanize(run.status)}</strong><span>{new Date(run.requested_at).toLocaleString()}</span></div>
              ...
              <p>Progress: {progressSummary(run)}{run.current_source_id ? ` · Current: ${sourceLabel(run, run.current_source_id)}` : ""}{run.cancel_requested ? " · cancellation requested" : ""}</p>
              ...
              {(run.status === "completed" || run.status === "completed_with_gaps") && <ProfessionalEvidenceReview apiBase={apiBase} runId={run.id} />}
              {run.stop_reason && <p>{humanize(run.stop_reason)}</p>}
              {run.status !== "running" && deletingRunId !== run.id && (
                <button type="button" className="danger" disabled={busy} onClick={() => { setDeletingRunId(run.id); setDeletePhrase(""); }}>Delete this capture batch</button>
              )}
              {deletingRunId === run.id && (
                <div className="professional-delete-confirmation">
                  ...
                  <label>Type {DELETE_PHRASE}<input aria-label={`Deletion phrase for ${run.id}`} value={deletePhrase} onChange={(event) => setDeletePhrase(event.target.value)} /></label>
