...
def test_archiving_capture_batch_hides_unreviewed_import_from_opportunity_inbox(tmp_path) -> None:
    ...
    archived = client.post(f"/api/captures/{captured['capture_run_id']}/archive")
    assert archived.status_code == 200
    assert archived.json()["status"] == "archived"
    assert archived.json()["hidden_pending_posting_count"] == 1
    assert archived.json()["protected_posting_count"] == 0

    assert client.get("/api/opportunity-index").json() == []
    captures = client.get("/api/captures").json()
    assert len(captures) == 1
    assert captures[0]["status"] == "archived"


def test_reviewed_capture_import_leaves_inbox_and_is_protected_when_archived(tmp_path) -> None:
    client = TestClient(create_app(f"sqlite:///{(tmp_path / 'jolt.db').as_posix()}"))
    ...
    assert archived.status_code == 200
    assert archived.json()["status"] == "archived"
    assert archived.json()["hidden_pending_posting_count"] == 0
    assert archived.json()["protected_posting_count"] == 1

    assert client.get("/api/opportunity-index").json() == []
    tracked = client.get("/api/application-index").json()
    assert len(tracked) == 1
    assert tracked[0]["posting_id"] == opportunity["posting_id"]
