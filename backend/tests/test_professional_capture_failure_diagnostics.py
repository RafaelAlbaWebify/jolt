...
def test_source_capture_crash_without_diagnostic_write_marks_progress(tmp_path: Path) -> None:
    client, factory = _client_and_factory(tmp_path)
    evidence_root = tmp_path / "evidence"
    configured = client.post(
        "/api/professional-intelligence/evidence-root",
    ...
    created = client.post(
        "/api/professional-intelligence/capture-runs",
        json={"options": {"max_sources": 1, "timeout_seconds": 10, "stop_on_failure": True}},
    )
    assert created.status_code == 200
    run = created.json()
    run_id = run["id"]
    source_url = run["planned_sources"][0]["url"]
    ...
    # Force diagnostic artifact writing to fail by replacing the expected evidence
    # directory with a file at the same path after the root was configured.
    ...
    for child in professional_root.iterdir():
        child.unlink()
    professional_root.rmdir()
    professional_root.write_text("not a directory", encoding="utf-8")
    ...
    assert response.source_progress[0].status == "failed"
    assert "Source capture crashed before diagnostics were committed" in response.source_progress[0].detail
    assert source_url in response.source_progress[0].detail
    assert "synthetic browser failure" in response.source_progress[0].detail
    assert response.stop_reason == "source_capture_engine_failure"
