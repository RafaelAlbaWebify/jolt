...
def test_profile_section_capture_traverses_intermediate_lazy_load_triggers() -> None:
    ...
    <html>
      <body style="margin:0">
        <main id="items"></main>
        <div style="height:5000px">scroll sentinel</div>
        <script>
          ...
          function addBatch() {
            for (let i = 1; i <= 3; i += 1) {
              ...
              card.textContent = `Credential ${loaded}`;
          ...
          window.addEventListener('scroll', () => {
            ...
            const lower = windows[stage];
            const upper = lower + 350;
...
def test_profile_section_capture_scrolls_until_bottom_content_stabilizes() -> None:
    html = """
    <html>
      <body style="margin:0">
        <main id="items"></main>
