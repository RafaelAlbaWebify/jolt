...
def create_app(database_url: str | None = None) -> FastAPI:
    ...
    def manual_intake(
        request: ManualIntakeRequest, session: Annotated[Session, Depends(get_session)]
    ) -> IntakeResponse:
    ...
    def linkedin_fixture_capture(
        request: LinkedInFixtureCaptureRequest, session: Annotated[Session, Depends(get_session)]
    ) -> CaptureRunResponse:
    ...
    def linkedin_live_capture(
        request: LinkedInLiveCaptureRequest, session: Annotated[Session, Depends(get_session)]
    ) -> CaptureRunResponse:
    ...
    def capture_run(
        capture_run_id: str, session: Annotated[Session, Depends(get_session)]
    ) -> CaptureRunResponse:
        try:
    ...
    def create_review(
        posting_id: str, request: ReviewRequest, session: Annotated[Session, Depends(get_session)]
    ) -> ReviewResponse:
        try:
    ...
    def identity_evidence(
        posting_id: str, session: Annotated[Session, Depends(get_session)]
    ) -> dict[str, object]:
        try:
    ...
    def application(
        application_id: str, session: Annotated[Session, Depends(get_session)]
    ) -> ApplicationResponse:
        try:
    ...
    def opportunity_index(
        session: Annotated[Session, Depends(get_session)], include_reviewed: bool = False
