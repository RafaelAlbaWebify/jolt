...
from jolt.capture_deletion import (
    CaptureRunDeletionResult,
    delete_capture_run_with_unclassified_opportunities,
)
...
def create_app(database_url: str | None = None) -> FastAPI:
    ...
    @app.post(
        "/api/captures/{capture_run_id}/delete",
        response_model=CaptureRunDeletionResult,
        tags=["captures"],
    )
    def delete_capture_batch(
        capture_run_id: str,
        session: Annotated[Session, Depends(get_session)],
    ) -> CaptureRunDeletionResult:
        try:
            return delete_capture_run_with_unclassified_opportunities(session, capture_run_id)
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
