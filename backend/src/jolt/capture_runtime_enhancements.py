...
def _active_filter_labels(page: Page) -> list[str]:
    ...
    for selector in (
        ...
        "button[aria-label*='filter' i][aria-pressed='true']",
    ):
        locator = page.locator(selector)
        try:
        ...
        for index in range(count):
            text = safe_text(locator.nth(index))
    ...
    *,
    ...
    for index in range(count):
        ...
        if not source_job_id:
            skipped.append(multipage_capture.SkippedCard(page_number, index, "", "Card had no usable LinkedIn job identity."))
            continue
        if source_job_id in seen:
            skipped.append(multipage_capture.SkippedCard(page_number, index, source_job_id, "Duplicate job identity across pages."))
            continue
        if title_link is None:
            skipped.append(multipage_capture.SkippedCard(page_number, index, source_job_id, "Card had no supported title link."))
            continue
        seen.add(source_job_id)
        title = safe_text(title_link)
        company_locator = multipage_capture._first_existing(card, multipage_capture.COMPANY_SELECTORS)
        location_locator = multipage_capture._first_existing(card, multipage_capture.LOCATION_SELECTORS)
        ...
        if not _click_with_one_retry(title_link, card):
            captured.append(CapturedCard(source_job_id, source_url, title, company, location, "", "", False, "Listing click timed out after one retry."))
            continue
        ...
        with contextlib.suppress(Exception):
            page.screenshot(path=evidence_dir / f"page_{page_number}_job_{source_job_id}.png", full_page=False)
        captured.append(CapturedCard(source_job_id, source_url, title, company, location, detail_html, description, verified, reason))
    return captured
...
def submit_capture(api_url: str, cards: list[CapturedCard], search_url: str, requested_item_limit: int, stop_reason: str) -> dict[str, Any]:
    payload = json.dumps({
    ...
    request = urllib.request.Request(f"{api_url.rstrip('/')}/api/captures/linkedin/live", data=payload, headers={"Content-Type": "application/json"}, method="POST")
    try:
        ...
        if run_log.exists():
            with run_log.open("a", encoding="utf-8") as stream:
                stream.write(
                    ...
                    f"\nRecovered after retry: {_retry_metrics['recovered_after_retry_count']}"
