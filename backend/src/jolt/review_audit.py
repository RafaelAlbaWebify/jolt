...
def audit(output_dir: Path) -> dict[str, object]:
    ...
    for item in opportunities:
        ...
        if item.get("profile_version_id") != EXPECTED_PROFILE:
            findings.append({"severity": "error", "message": f"{title}: unexpected profile version."})
        if item.get("engine_version") != EXPECTED_ENGINE:
            findings.append({"severity": "error", "message": f"{title}: unexpected engine version."})
        score = item.get("ranking_score")
        if not isinstance(score, int) or not 0 <= score <= 100:
            findings.append({"severity": "error", "message": f"{title}: invalid ranking score."})
        if item.get("review_decision") and item.get("proposed_decision") != item.get("review_decision"):
            findings.append({
                "severity": "info",
        ...
        if not item.get("strengths") and not item.get("gaps"):
            findings.append({"severity": "warning", "message": f"{title}: no strengths or gaps recorded."})
    ...
    if page_errors:
        findings.extend({"severity": "error", "message": f"Browser error: {error}"} for error in page_errors)
