...
def audit_workbench(api_url: str, app_url: str, output_dir: Path) -> dict[str, object]:
        ...
        for index, opportunity in enumerate(opportunities, start=1):
            ...
            if not title_visible:
                findings.append(
                    {
                        "severity": "error",
                        "message": f"Opportunity title is missing from rendered workbench: {title or posting_id}.",
                    }
                )
    ...
    if not visible_strategy_labels and opportunities:
        findings.append(
            {
                "severity": "warning",
                "message": "The API exposes strategy fields, but the rendered workbench does not visibly label them.",
            }
        )
