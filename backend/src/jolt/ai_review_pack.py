...
def _analysis_text(*, title: str, company: str, location: str, description: str, source_raw_text: str) -> str:
    evidence = source_raw_text.strip() or description.strip()
    cleaned = sanitize_capture_text(evidence)
    header = "\n".join((f"Title: {title.strip()}", f"Company: {company.strip()}", f"Location: {location.strip()}"))
...
def build_ai_review_pack(session: Session) -> bytes:
    ...
    pages = list(session.scalars(select(CapturePage).where(CapturePage.capture_run_id == capture.id).order_by(CapturePage.page_number, CapturePage.id)).all())
    items = list(session.scalars(select(CaptureItem).where(CaptureItem.capture_run_id == capture.id).order_by(CaptureItem.id)).all())

    posting_ids = {item.posting_id for item in items if item.posting_id is not None}
    postings = list(session.scalars(select(Posting).where(Posting.id.in_(posting_ids)).order_by(Posting.created_at, Posting.id)).all()) if posting_ids else []
    posting_by_id = {posting.id: posting for posting in postings}

    source_document_ids = {item.source_document_id for item in items if item.source_document_id is not None}
    source_document_ids.update(posting.source_document_id for posting in postings)
    source_documents = list(session.scalars(select(SourceDocument).where(SourceDocument.id.in_(source_document_ids)).order_by(SourceDocument.captured_at, SourceDocument.id)).all()) if source_document_ids else []
    ...
    for item in items:
        ...
        source_document_id = posting.source_document_id if posting is not None else item.source_document_id
        ...
        jobs_payload.append({
            ...
            "analysis_text": _analysis_text(title=title, company=company, location=location, description=description, source_raw_text=source_raw_text),
            "audit": {
                "source_raw_text": source_raw_text,
                "source_raw_text_sha256": hashlib.sha256(source_raw_text.encode("utf-8")).hexdigest(),
            },
        })
    ...
    response_template = {
        ...
        "policy_version": preferences.review_policy_version,
        "capture_run_id": capture.id,
        ...
        "jobs": [{
            ...
            "hard_blockers": [{"blocker_type": "language|geography|work_authorization|citizenship|clearance|duplicate|other_legal", "evidence": "Exact vacancy evidence for the blocker"}],
            "transferable_skills": [],
            "skill_gaps": [],
    ...
    readme = f"""# JOLT AI Review + Market Insights Package

One package, one external AI analysis round, one return import.
Policy version: {preferences.review_policy_version}

Critical review policy:
- A foreign LinkedIn/listing location is neutral by itself.
- Explicit remote scope such as `Remote (USA)` or `remote within Germany` is positive geography restriction evidence.
- Required distant office/client-site work is a hard blocker because the candidate will not relocate.
- Record CEFR proficiency separately from certificate requirements; require a certificate only when explicitly requested.
- Shifts/weekends/holidays are not rejection criteria.
- Technical fit is scored only after hard blockers are checked.
- Return the unchanged `policy_version`; JOLT marks reviews from older policies stale.
...
"""
    ...
    manifest = {
        ...
        "review_policy_version": preferences.review_policy_version,
