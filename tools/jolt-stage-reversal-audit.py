...
def open_workflow(workspace: Locator, observations: dict[str, object], key: str) -> Locator:
    toggle = workspace.get_by_role("button", name="Manage application", exact=False)
    toggle.wait_for(state="visible", timeout=30_000)
    toggle.click()
    workspace.locator(".application-workflow-panel").wait_for(state="visible", timeout=30_000)
    current_stage = workspace.locator(".workflow-current-stage h4")
    current_stage.wait_for(state="visible", timeout=30_000)
    observations[key] = toggle.get_attribute("aria-expanded") != "true"
    return current_stage


def workflow_controls(workspace: Locator) -> tuple[Locator, Locator, Locator]:
    workflow = workspace.locator(".application-workflow-panel")
    stage_select = workflow.locator(".workflow-outcome-section").filter(has_text="Change stage").locator("select")
    notes = workflow.locator("label.workflow-notes textarea")
    outcome_select = workflow.locator(".workflow-outcome-section").filter(has_text="Final outcome").locator("select")
    stage_select.wait_for(state="visible", timeout=30_000)
    notes.wait_for(state="visible", timeout=30_000)
    return stage_select, notes, outcome_select
...
def main() -> int:
    ...
    summary: dict[str, object] = {
        "started_at": now_iso(), "app_url": APP_URL, "viewport": VIEWPORT,
        "console": [], "page_errors": [], "failed_requests": [], "responses": [],
        "screenshots": [], "observations": {}, "ux_checks": {},
    }
    ...
    with sync_playwright() as playwright:
        ...
        page.on("console", lambda message: summary["console"].append({"type": message.type, "text": message.text, "timestamp": now_iso()}))
        page.on("pageerror", lambda error: summary["page_errors"].append({"message": str(error), "timestamp": now_iso()}))
        page.on("requestfailed", lambda request: summary["failed_requests"].append({"method": request.method, "url": request.url, "failure": request.failure, "timestamp": now_iso()}))

        def record_response(response: Response) -> None:
            if response.url.startswith(API_URL):
                summary["responses"].append({"status": response.status, "method": response.request.method, "url": response.url})

        page.on("response", record_response)
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        title = f"JOLT Stage Reversal Audit {stamp}"
        intake = page.request.post(f"{API_URL}/api/intake/manual", data={
        ...
        review = page.request.post(f"{API_URL}/api/opportunities/{intake_data['posting_id']}/reviews", data={
            "evaluation_id": intake_data["evaluation_id"], "decision": "pursue",
            "reason_code": "playwright_audit", "notes": "Supervised stage-reversal acceptance fixture.",
        })
        if not review.ok:
            raise RuntimeError(f"Audit fixture review failed: {review.status}")
        application = page.request.post(f"{API_URL}/api/opportunities/{intake_data['posting_id']}/applications", data={
        ...
        stage_select, notes, outcome_select = workflow_controls(workspace)
        ...
        current_stage.get_by_text("technical interview", exact=True).wait_for(state="visible", timeout=30_000)
        ...
        outcome_select.wait_for(state="visible", timeout=30_000)
        outcome_select.select_option("rejected_by_employer")
        workspace.get_by_role("button", name="Record final outcome").click()
        workspace.get_by_text("Final outcome: rejected by employer", exact=True).wait_for(state="visible", timeout=30_000)
        ...
        workspace.get_by_role("button", name="Save stage").click()
        current_stage.get_by_text("recruiter screen", exact=True).wait_for(state="visible", timeout=30_000)
        ...
        interviewing_card = page.locator(".application-lane-interviewing .application-card", has_text=title)
        ...
        workspace = page.get_by_role("dialog")
        workspace.wait_for(state="visible", timeout=30_000)
        persisted_stage = open_workflow(workspace, observations, "workflow_collapsed_after_reopen_load")
        persisted_stage.get_by_text("recruiter screen", exact=True).wait_for(state="visible", timeout=30_000)
        ...
        expected_fragments = ["technical interview", "submitted", "rejected by employer", "recruiter screen"]
        observations["expected_timeline_fragments"] = {fragment: fragment in timeline_text for fragment in expected_fragments}
        snapshot(page, output_dir, "08-timeline-stage-reversal", summary)

        ux_checks["horizontal_overflow_px"] = horizontal_overflow(page)
        ux_checks["workspace_horizontal_overflow_px"] = int(workspace.evaluate("element => Math.max(0, element.scrollWidth - element.clientWidth)"))
        ux_checks["all_expected_timeline_fragments"] = all(observations["expected_timeline_fragments"].values())
        ux_checks["reopened_card_in_interviewing_lane"] = interviewing_card.count() == 1
        ux_checks["workflow_disclosure_stable"] = not bool(observations["workflow_collapsed_after_initial_load"] or observations["workflow_collapsed_after_reopen_load"])
    ...
    (output_dir / "audit-summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
