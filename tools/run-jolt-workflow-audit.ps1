...
$AuditRoot = Join-Path $RepoRoot ".jolt\workflow-audits\$Timestamp"
$Downloads = Join-Path $HOME "Downloads"
$ZipPath = Join-Path $Downloads "JOLT_WORKFLOW_AUDIT_$Timestamp.zip"
...
Write-Host "Running read-only workflow audit..."

Push-Location $BackendRoot
try {
    & uv run python (Join-Path $RepoRoot "tools\jolt-workflow-audit.py") $AuditRoot
